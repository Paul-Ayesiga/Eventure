    ...
    <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
        <div class="flex justify-between items-center mb-6">
            ...
            <div class="flex flex-wrap gap-2">
                <div class="flex space-x-2">
                    <flux:button icon="plus" wire:click="$set('isSimulating', true)" variant="primary" class="bg-teal-500">
                        Simulate Purchase
                    </flux:button>
                </div>
                <div class="flex space-x-2">
        ...
        @if($bookings->isEmpty())
            <div class="text-center py-8">
                ...
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Get started by simulating a purchase or wait for customers to book tickets.</p>
                <div class="mt-6">
                    <flux:button icon="plus" wire:click="$set('isSimulating', true)" variant="primary" class="bg-teal-500">
                        Simulate Purchase
                    </flux:button>
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead class="bg-gray-50 dark:bg-gray-700">
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Tickets</th>
                    ...
                    <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        @foreach($bookings as $booking)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @if($booking->attendees->isNotEmpty())
                                        <div class="text-sm font-medium text-gray-900 dark:text-white">
                                            {{ $booking->attendees->first()->first_name }} {{ $booking->attendees->first()->last_name }}
                                        </div>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-900 dark:text-white">{{ number_format($booking->total_amount, 2) }} {{ $event->currency }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $booking->status === 'confirmed' ? 'bg-green-100 text-green-800 dark:bg-green-800 dark:text-green-100' : 'bg-yellow-100 text-yellow-800 dark:bg-yellow-800 dark:text-yellow-100' }}">
                                        {{ ucfirst($booking->status) }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-900 dark:text-white">{{ $booking->created_at->format('M d, Y') }}</div>
                                    <div class="text-sm text-gray-500 dark:text-gray-400">{{ $booking->created_at->format('h:i A') }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex justify-end space-x-2">
                                        <flux:button.group>
                                            <flux:button size="sm" icon="eye" wire:click="viewBooking({{ $booking->id }})" variant="primary" title="View Details">
                                            </flux:button>
                                            <flux:button size="sm" icon="printer" wire:click="printBooking({{ $booking->id }})" variant="outline" title="Print">
                                            </flux:button>
                                            <flux:modal.trigger :name="'delete-booking-' . $booking->id">
                                                <flux:button variant="danger" icon="trash" size="sm" title="Delete"></flux:button>
                                            </flux:modal.trigger>
                                        </flux:button.group>
                                    </div>
                                </td>
                            ...
                            <flux:modal :name="'delete-booking-'.$booking->id" class="min-w-[22rem]">
                                <div class="space-y-6">
                                    ...
                                    <div class="flex gap-2">
                                        ...
                                        <flux:button variant="danger" wire:click="deleteBooking({{ $booking->id }})" class="cursor-pointer">Delete Booking</flux:button>
                                    </div>
                                </div>
                            </flux:modal>
                        @endforeach
            ...
            <form wire:submit.prevent="simulatePurchase">
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        @error('selectedCustomerId') <span class="text-red-500 text-sm mt-1">{{ $message }}</span> @enderror
                    </div>

                    <!-- Ticket Selection -->
                    <div>
                        <flux:label for="selectedTicketId" required>Select Ticket</flux:label>
                        <flux:select id="selectedTicketId" wire:model="selectedTicketId" class="mt-1 w-full">
                            <option value="">Select a ticket</option>
                            @foreach($tickets as $ticket)
                                <option value="{{ $ticket->id }}">
                                    {{ $ticket->name }} - {{ number_format($ticket->price, 2) }} {{ $event->currency }}
                        ...
                        @error('selectedTicketId') <span class="text-red-500 text-sm mt-1">{{ $message }}</span> @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <flux:input
                            id="ticketQuantity"
                            type="number"
                            wire:model="ticketQuantity"
                            class="mt-1 w-full"
                            min="1"
                            :max="$selectedTicketId ? $tickets->firstWhere('id', $selectedTicketId)?->max_tickets_per_booking : null"
                        />
                        @error('ticketQuantity') <span class="text-red-500 text-sm mt-1">{{ $message }}</span> @enderror
                    </div>
                    ...
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-sm text-gray-500 dark:text-gray-400">Get notified when tickets become available</p>
                        </div>
        ...
        <div class="p-6">
            ...
            @if($selectedBooking)
                <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <p class="text-sm text-gray-500 dark:text-gray-400">{{ $selectedBooking->booking_reference }}</p>
                        </div>
                        <div>
                            <h4 class="text-sm font-medium text-gray-500 dark:text-gray-400">Booking Date</h4>
                            <p class="mt-1 text-sm text-gray-900 dark:text-white">{{ $selectedBooking->created_at->format('M d, Y h:i A') }}</p>
                        </div>
                        <div>
                            <h4 class="text-sm font-medium text-gray-500 dark:text-gray-400">Status</h4>
                            <p class="mt-1">
                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $selectedBooking->status === 'confirmed' ? 'bg-green-100 text-green-800 dark:bg-green-800 dark:text-green-100' : 'bg-yellow-100 text-yellow-800 dark:bg-yellow-800 dark:text-yellow-100' }}">
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-sm text-gray-900 dark:text-white">{{ number_format($selectedBooking->total_amount, 2) }} {{ $event->currency }}</p>
                        </div>
                    </div>
                            ...
                            @foreach($selectedBooking->tickets as $ticket)
                                <div class="flex justify-between items-center p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                                    <div>
                                        ...
                                        <p class="text-sm text-gray-500 dark:text-gray-400">Quantity: {{ $ticket->pivot->quantity }}</p>
                                    </div>
                                    <div class="text-right">
                                        <p class="text-sm font-medium text-gray-900 dark:text-white">{{ number_format($ticket->price * $ticket->pivot->quantity, 2) }} {{ $event->currency }}</p>
                                        <p class="text-sm text-gray-500 dark:text-gray-400">{{ number_format($ticket->price, 2) }} each</p>
                                    </div>
                                </div>
                            @endforeach
                        </div>
                ...
                <div class="mt-6 flex justify-end space-x-3">
                    ...
                    <flux:button type="button" icon="printer" variant="primary" wire:click="printBooking({{ $selectedBooking->id }})">
                        Print
                    </flux:button>
                </div>
            @endif
