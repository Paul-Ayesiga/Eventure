...
new class extends Component {
    ...
    {
        $this->reset([
            'eventType', 'name', 'venue', 'start_date',
...
<div>
      <!-- Search and filters header -->
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-4">
            <div class="flex flex-col sm:flex-row gap-4 w-full sm:w-auto items-start sm:items-center">
                ...
                <div class="relative w-full sm:w-60">
                    <input wire:model="search" wire:keyup="fetchEvents" type="text" placeholder="Search" class="pl-10 pr-4 py-2 border rounded-lg w-full dark:bg-black dark:border-gray-700 dark:text-gray-200 dark:placeholder-gray-400">
                    <svg wire:loading.remove wire:target="search" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                    </svg>
                    <svg class="h-5 w-5 absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400" wire:loading wire:target="search" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200"><circle fill="none" stroke-opacity="1" stroke="#FF156D" stroke-width=".5" cx="100" cy="100" r="0"><animate attributeName="r" calcMode="spline" dur="2" values="1;80" keyTimes="0;1" keySplines="0 .2 .5 1" repeatCount="indefinite"></animate><animate attributeName="stroke-width" calcMode="spline" dur="2" values="0;25" keyTimes="0;1" keySplines="0 .2 .5 1" repeatCount="indefinite"></animate><animate attributeName="stroke-opacity" calcMode="spline" dur="2" values="1;0" keyTimes="0;1" keySplines="0 .2 .5 1" repeatCount="indefinite"></animate></circle></svg>

                </div>

                <!-- Event filter dropdown -->
                <div class="w-full sm:w-auto relative">
                    <select wire:model="eventTypeSelected" wire:change="fetchEvents" placeholder="Choose event..." class="w-full border rounded p-2">
            ...
            <div class="flex gap-2 w-full sm:w-auto justify-end">
                <flux:modal.trigger name="create-event">
                    <flux:button variant="primary" class="border-none outline-none cursor-pointer bg-teal-500">Create Event</flux:button>
                </flux:modal.trigger>

                <!-- Modal for creating event -->
                <flux:modal name="create-event" >
                    <div class="space-y-8">
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-2 gap-4 mt-4">
                                ...
                                <div>
                                    ...
                                    <div class="text-sm text-gray-400 mt-1">Event ends on [date]</div>
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-4 mt-4">
                                <div>
                                    ...
                                    <select wire:model="timezone" class="w-full border rounded p-2 mt-2">
                                        ...
                                        <!-- More timezones as needed -->
                                    </select>
                                    @error('timezone') <span class="text-red-500 text-sm mt-1">{{ $message }}</span> @enderror
                                </div>
                                <div>
                ...
                <flux:button  wire:click="refresh" class="bg-gray-100 dark:bg-gray-700 p-2 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-600">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 dark:text-gray-200" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div class="relative">
             <!-- Loading Overlay -->
            <div wire:loading wire:target="eventTypeSelected, search, refresh" class="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-50">
            ...
            @forelse ($events as $event)
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow hover:shadow-lg transition-shadow duration-300 cursor-pointer mb-4" wire:key="{{ $event->id }}">
                    <!-- Event card -->
                        ...
                        <div class="flex items-start">
                            ...
                            <div class="w-24 h-16 bg-gray-200 dark:bg-gray-700 rounded-lg mr-4 overflow-hidden">
                                @if($event->banners && count(json_decode($event->banners, true)) > 0)
                                    <img
                                        src="{{ json_decode($event->banners, true)[0] }}"
                                        alt="{{ $event->name }}"
                            ...
                            <div class="flex-1">
                                <div class="flex justify-between">
                                    ...
                                    <!-- Options button -->
                                    <flux:dropdown>
                                        <flux:button variant="ghost" icon="ellipsis-vertical" class="p-1"></flux:button>

                                        <flux:menu>
                                            <flux:menu.item :href="route('event-details', ['id' => $event->id])" wire:navigate>Manage</flux:menu.item>
                                            ...
                                            <flux:modal.trigger :name="'delete-event-'.$event->id">
                                                <flux:menu.item variant="danger" :name="'delete-event-'.$event->id">delete</flux:menu.item>
                                            </flux:modal.trigger>
                                            @teleport('body')
                                            <!-- Delete Event Modal -->
                                                <flux:modal :name="'delete-event-'.$event->id" class="min-w-[22rem]">
                                                    <div class="space-y-6">
                                                        ...
                                                        <div class="flex gap-2">
                                                            ...
                                                            <flux:button variant="danger" wire:click="delete({{ $event->id }})" class="cursor-pointer">Delete Event</flux:button>
                                                        </div>
                                                    </div>
                                                </flux:modal>
                                            @endteleport
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-4">
                            ...
                            <div class="flex items-center">
                                ...
                                <div>
                                    ...
                                    <div class="text-gray-900 dark:text-gray-200 text-sm">{{ $event->start_datetime}} ({{ $event->timezone }})</div>
                                </div>
                            </div>
