                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead class="bg-gray-50 dark:bg-gray-700">
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Sale Period</th>
                        ...
                        @foreach($tickets as $ticket)
                            <tr>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-gray-900 dark:text-white">{{ $ticket->name }}</div>
                                    @if($ticket->description)
                                        <div class="text-sm text-gray-500 dark:text-gray-400 truncate max-w-xs">{{ $ticket->description }}</div>
                                    @endif
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-900 dark:text-white">{{ number_format($ticket->price, 2) }} {{ $event->currency }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-900 dark:text-white">{{ $ticket->getRemainingQuantity() }} / {{ $ticket->quantity_available }}</div>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-900 dark:text-white">{{ $ticket->sale_start_date->format('M d, Y') }}</div>
                                    <div class="text-sm text-gray-500 dark:text-gray-400">to {{ $ticket->sale_end_date->format('M d, Y') }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $ticket->status === 'active' ? 'bg-green-100 text-green-800 dark:bg-green-800 dark:text-green-100' : 'bg-red-100 text-red-800 dark:bg-red-800 dark:text-red-100' }}">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex justify-end space-x-2">
                                        <flux:button.group>
                                            <flux:button size="sm" icon="pencil" wire:click="openEditModal({{ $ticket->id }})" variant="primary">
                                            </flux:button>
                                            <flux:button size="sm" wire:click="toggleTicketStatus({{ $ticket->id }})" variant="subtle">
                            ...
                            <flux:modal :name="'delete-ticket-'.$ticket->id" class="min-w-[22rem]">
                                <div class="space-y-6">
                                                        ...
                                                        <div class="flex gap-2">
                                                            ...
                                                            <flux:button variant="danger" wire:click="deleteTicket({{ $ticket->id }})" class="cursor-pointer">Delete Ticket</flux:button>
                                                        </div>
                                </div>
                            </flux:modal>
                        @endforeach
                    ...
                    <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-4">
                        ...
                        <div class="mt-4 space-y-2">
                            ...
                            <div>
                                ...
                                <div class="text-sm text-gray-500 dark:text-gray-400">
                                    {{ $ticket->sale_start_date->format('M d, Y') }} to {{ $ticket->sale_end_date->format('M d, Y') }}
                                </div>
                            </div>
                        </div>
            ...
            <form wire:submit.prevent="saveTicket">
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <flux:input id="name" wire:model="name" type="text" class="mt-1 w-full" placeholder="e.g. VIP, Regular, Early Bird" />
                    ...
                    <div>
                        ...
                        <flux:textarea id="description" wire:model="description" class="mt-1 w-full" rows="3" placeholder="Describe what this ticket includes"></flux:textarea>
                    ...
                    <div>
                        ...
                        <div class="mt-1 relative rounded-md shadow-sm">
                            ...
                            <flux:input id="price" wire:model="price" type="number" step="0.01" min="0" class="pl-12 w-full" placeholder="0.00" />
                        </div>
                    ...
                    <div>
                        ...
                        <flux:input id="quantity_available" wire:model="quantity_available" type="number" min="1" class="mt-1 w-full" placeholder="100" />
                        @error('quantity_available') <span class="text-red-500 text-sm mt-1">{{ $message }}</span> @enderror
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <flux:input id="sale_start_date" wire:model="sale_start_date" type="date" class="mt-1 w-full" />
                            @error('sale_start_date') <span class="text-red-500 text-sm mt-1">{{ $message }}</span> @enderror
                        </div>
                        <div>
                            <flux:label for="sale_end_date" required>Sale End Date</flux:label>
                            <flux:input id="sale_end_date" wire:model="sale_end_date" type="date" class="mt-1 w-full" />
                            @error('sale_end_date') <span class="text-red-500 text-sm mt-1">{{ $message }}</span> @enderror
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <flux:input id="max_tickets_per_booking" wire:model="max_tickets_per_booking" type="number" min="1" class="mt-1 w-full" placeholder="1" />
                        @error('max_tickets_per_booking') <span class="text-red-500 text-sm mt-1">{{ $message }}</span> @enderror
                    </div>
